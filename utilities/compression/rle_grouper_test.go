...
var basicTestCases = []BasicTestCase{
	{[]byte{}, InvalidRLERun, "empty"},
	{[]byte{0, 0, 1, 0, 0, 0, 0}, ByteRun{Byte: byte(0), RunLength: 2}, "two initial"},
	{[]byte{6, 1, 5, 20, 31}, ByteRun{Byte: byte(6), RunLength: 1}, "one byte"},
	{[]byte{9, 9, 9, 9, 9, 9}, ByteRun{Byte: byte(9), RunLength: 6}, "entire run"},
}

func runBasicTestCase(t *testing.T, test BasicTestCase) {
	grouper := NewRLEGrouper(bytes.NewBuffer(test.Data))
...
func TestRLEGrouper__Sequence(t *testing.T) {
	...
	expected := []ByteRun{
		...
		{byte(1), 1}, {byte(0), 3}, InvalidRLERun,
	}

	buffer := bytes.NewBuffer(data)
	grouper := NewRLEGrouper(buffer)
	for i, expectedRun := range expected {
		...
		if expectedRun == InvalidRLERun && err != io.EOF {
